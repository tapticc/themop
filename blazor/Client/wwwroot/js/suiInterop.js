...
function extractOptionValue(value) {
  ...
  if (value.Some !== void 0)
    return value.Some;
  if (value.None !== void 0)
    return null;
  return value;
}
...
async function onlineStorageUnit(args) {
  const { network } = requireInit();
  const tx = new Transaction3();
  tx.setGasBudget(5e7);
...
async function offlineStorageUnit(args) {
  const { network } = requireInit();
  const tx = new Transaction3();
  tx.setGasBudget(5e7);
  console.log("storageUnitId", args.storageUnitId);
...
async function authorizeSmartStorageExtension(args) {
  const { network } = requireInit();
  const tx = new Transaction3();
  tx.setGasBudget(5e7);
  const [storageOwnerCap, returnReceipt] = tx.moveCall({
...
async function updateStorageUnitName(args) {
  const { network } = requireInit();
  const tx = new Transaction3();
  tx.setGasBudget(5e7);
  const [storageOwnerCap, returnReceipt] = tx.moveCall({
...
async function updateStorageUnitDescription(args) {
  const { network } = requireInit();
  const tx = new Transaction3();
  tx.setGasBudget(5e7);
  const [storageOwnerCap, returnReceipt] = tx.moveCall({
...
async function updateStorageUnitUrl(args) {
  const { network } = requireInit();
  const tx = new Transaction3();
  tx.setGasBudget(5e7);
  const [storageOwnerCap, returnReceipt] = tx.moveCall({
...
async function getGate(gateId) {
  try {
    ...
    const linkedGate = fields?.linked_gate_id?.fields?.vec?.[0] ?? fields?.linked_gate_id?.vec?.[0] ?? "";
    return {
      found: true,
...
async function configureGateAssembly(args) {
  const { network } = requireInit();
  const tx = new Transaction3();
  tx.setGasBudget(5e7);
...
async function getGatePermitConfig(args) {
  try {
    const client = JsonClient();
    const result = await client.getObject({
      id: args.registryId,
    ...
    return {
      ...
      linkedPointsRegistryId: fields?.linked_points_registry_id?.fields?.vec?.[0] ?? fields?.linked_points_registry_id ?? "",
      rawJson: JSON.stringify(fields, null, 2)
    };
  } catch (err) {
    return {
...
async function purchaseJumpPermit(args) {
  const { network } = requireInit();
  const tx = new Transaction3();
  tx.setGasBudget(5e7);
  tx.moveCall({
...
export {
  ...
  setItemConfig,
  setResourceConfig,
  updateStorageUnitDescription,
  updateStorageUnitName,
  updateStorageUnitUrl
};
